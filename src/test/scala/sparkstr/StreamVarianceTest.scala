...
class StreamVarianceTest extends FunSuite {
  ...
  test("combinePrevCurrVars zero Vars") {
    ...
    val expectedVar = 0.0
    val result:Double = UpdateFunction.combinePrevCurrVars(prevVar, prevMean, prevCount, 
  ...
  test("combinePrevCurrVars with known variances:") {
    ...
    val prevVar = UpdateFunction.sumSquaresVariance(values = values.take(3))
    assert(0.6666666666666679===prevVar)
    val prevMean = values.take(3).sum/3
    assert(-4===prevMean)
    val prevCount = 3
    val currVar = UpdateFunction.sumSquaresVariance(values = values.drop(3))
    assert(26.91666666666667===currVar)
    val currMean = values.drop(3).sum/(21-3)
    assert(6.5===currMean)
    val currCount = 21-3
    val expectedVar = UpdateFunction.sumSquaresVariance(values = values)  // = 36.6667
    val result:Double = UpdateFunction.combinePrevCurrVars(prevVar, prevMean, prevCount, 
                                                           currVar, currMean, currCount)
  ...
  test("combinePrevCurrVars in loop") {
    val values = (-5 to 15).map(_.toDouble)
    val expectedVar = UpdateFunction.sumSquaresVariance(values = values)
    for(i <- 1 to 20) {
      val prevVar = UpdateFunction.sumSquaresVariance(values = values.take(i))
      val prevMean = values.take(i).sum/i
      val prevCount = i
      val currVar = UpdateFunction.sumSquaresVariance(values = values.drop(i))
  ...
  test("sumSquaresVar() with known variance") {
    ...
    val result = UpdateFunction.sumSquaresVariance(prevVar, prevMean, prevCount, values)
  ...
  test("sumSquaresVar() prevVar=0, mean=2.") {
    ...
    val expected: Double = ((-0.0* -0.0 + 4*4 + -1* -1 + 5*5) / 4) - m * m // = 10.5-4=6.5
    assert(expected===result)
  }
